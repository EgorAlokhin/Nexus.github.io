import json
from database import cfg, setting_set, SessionLocal

DEFAULT_PREFS = {
    "notification_channel": "sms",  # SMS only by default; WhatsApp needs separate Twilio channel
...
def save_notification_prefs(prefs: dict) -> dict:
    ...
    db = SessionLocal()
    try:
        setting_set(db, "NOTIFICATION_PREFS", json.dumps(merged))
        db.commit()
    finally:
        db.close()
    return merged
