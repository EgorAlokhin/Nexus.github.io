...
from database import upsert_task, parse_due_datetime
from models import Task

# Strong academic signals — must appear in subject (or trusted school sender).
ACADEMIC_SUBJECT = (
...
JUNK_PHRASES = (
    ...
    "you're almost done! unlock", "what was your ap experience",  # spammy promos
)
...
def _extract_due_from_text(*parts):
    """Try to parse a due date from email subject/body; return None if not found."""
...
def _purge_junk_gmail(db):
    removed = 0
    for t in db.query(Task).filter(Task.source == "gmail").all():
