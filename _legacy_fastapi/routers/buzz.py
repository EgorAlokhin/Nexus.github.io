import httpx
from models import BUZZ_GRADABLE_TYPES, Task, UserSession
from database import upsert_task, parse_due_datetime, cfg
...
def _purge_buzz_lessons(db):
    gradable = list(BUZZ_GRADABLE_TYPES)
    db.query(Task).filter(
        Task.source == "buzz",
        ~Task.description.in_(gradable),
...
def sync_buzz(db):
    ...
    for ext, it in items.items():
        ...
        count += 1
    count += sync_buzz_activity(db, token, uid)
    db.commit()
    return count
...
def _mark_buzz_done_from_activity(db, enrollment_id, item_id):
    if not enrollment_id or not item_id:
        return
    ext = f"{enrollment_id}:{item_id}"
    from models import Task
    t = db.query(Task).filter(Task.source == "buzz", Task.external_id == ext).first()
    if t:
        t.is_completed = True


def sync_buzz_activity(db, token=None, uid=None):
    ...
    for enr in _as_list(gb, "enrollments"):
        ...
        for act in acts:
            ...
            if ng and iid and _grade_represents_completion(ng):
                _mark_buzz_done_from_activity(db, eid, iid)
            upsert_task(
                db,
                source="activity",
                external_id=ext,
            ...
            count += 1
    from models import Task
    for t in db.query(Task).filter(Task.source == "activity").all():
